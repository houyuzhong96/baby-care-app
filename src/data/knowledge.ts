...
// ======== 宝宝护理详细指南 (梅奥育儿全书 + 崔玉涛 + 实用程序育儿法) ========

export interface BabyCareTopic {
  title: string;
  icon: string;
  content: { subtitle: string; text: string }[];
}

export const babyCareGuide: BabyCareTopic[] = [
  {
    title: '新生儿脐带护理',
    icon: '🩹',
    content: [
      { subtitle: '保持干燥', text: '脐带通常在出生后1-3周脱落。脱落前保持脐带残端干燥、清洁。每次换尿布时将尿布向下折露出脐带区域。用棉签蘸75%酒精从脐带根部向外螺旋消毒，每天1-2次。' },
      { subtitle: '注意事项', text: '如脐带周围皮肤发红、肿胀、有脓液流出或有臭味需立即就医。脐带渗少量血是正常的。洗澡时避免脐带浸水，用海绵擦浴代替盆浴直到脐带脱落。' },
      { subtitle: '崔玉涛提醒', text: '不要用纱布包扎脐带。不要涂抹任何药膏或药粉。脐带脱落后肚脐处可能还有少量分泌物，继续用酒精消毒直至完全干燥。' },
    ]
  },
  {
    title: '新生儿喂养节奏',
    icon: '🍼',
    content: [
      { subtitle: '母乳喂养频率', text: '新生儿(0-6周)每2-3小时喂一次，每天8-12次。单侧每次15-20分钟。关注宝宝的饥饿信号而非按时喂养：咂嘴、吐舌头、觅食反射（碰触脸颊会转头）。哭是最后的饥饿信号，不要等到哭才喂。' },
      { subtitle: '吃饱的信号', text: '喂完后宝宝主动松开乳头/推开奶瓶，表情满足放松，手臂和手掌张开（不握拳）。每天6-8片湿尿布（淡黄色尿液），大便颜色正常（母乳喂养宝宝大便黄色糊状）。体重正常增长（新生儿每周增重150-200g）。' },
      { subtitle: '拍嗝方法', text: '每次喂奶后拍嗝。三种姿势：1)竖抱宝宝头部靠在肩上，手呈空杯状从下往上轻拍背部；2)让宝宝坐在大腿上，一手托住下巴和前胸，另一手拍背；3)让宝宝趴在腿上，头略高于身体轻拍。如5分钟内没打出嗝不必强求。' },
      { subtitle: '配方奶注意事项', text: '用70度以上热水冲泡奶粉以杀灭可能存在的阪崎肠杆菌。不要用微波炉加热——会形成热点烫伤宝宝。已冲泡的配方奶室温下存放不超过2小时。每公斤体重约150ml/天，分6-8次。' },
    ]
  },
  {
    title: '新生儿洗澡指南',
    icon: '🛁',
    content: [
      { subtitle: '洗澡频率', text: '新生儿不需要每天洗澡——每周2-3次足够，过于频繁会破坏皮肤天然屏障。脐带脱落前使用海绵擦浴（用湿毛巾擦拭身体各部位）。脐带脱落后可以盆浴。' },
      { subtitle: '洗澡步骤', text: '准备工作：室温26-28度，水温37-38度(手腕内侧试温)。先洗脸部(不用沐浴露)，再洗头，最后洗身体。用温和无香料的婴儿专用洗护产品。洗完后立即用柔软的浴巾包裹擦干，特别注意皮肤褶皱处。' },
      { subtitle: '安全要点', text: '绝对不要将宝宝单独留在浴盆中哪怕一秒钟。水深不超过5-8cm。所有洗澡用品提前准备好放在手边。洗完后涂婴儿润肤露保持皮肤滋润。' },
    ]
  },
  {
    title: '尿布护理',
    icon: '👶',
    content: [
      { subtitle: '更换频率', text: '新生儿每2-3小时更换一次尿布，大便后立即更换。每天约8-12片尿布。夜间可以少换（除非大便），使用吸收性好的夜间尿布。' },
      { subtitle: '预防尿布疹', text: '更换时用温水清洗臀部，用棉柔巾轻拍吸干(不要擦)。每次换尿布涂抹护臀膏(含氧化锌)形成保护屏障。每天让宝宝光屁股晾10-15分钟让皮肤呼吸。避免使用含酒精或香料的湿巾。' },
      { subtitle: '尿布疹处理', text: '出现红疹时增加换尿布频率。每次换后用温水冲洗并完全晾干。厚涂氧化锌护臀膏。如果持续3天不改善或出现水泡需就医(可能为真菌感染需用药膏)。' },
    ]
  },
  {
    title: '睡眠安全',
    icon: '😴',
    content: [
      { subtitle: '安全睡姿', text: '始终让宝宝仰卧睡觉（降低婴儿猝死综合征SIDS风险的唯一有效方法）。宝宝能自己翻身后（约4-6个月），如果他在睡眠中自己翻成俯卧不需要特意纠正——但放下去时仍需仰卧。' },
      { subtitle: '睡眠环境', text: '使用结实的婴儿床垫，铺紧床单。婴儿床内不放枕头、毛绒玩具、松软被褥、防撞垫——这些都会增加窒息风险。室温18-22度为宜。使用睡袋代替被子。' },
      { subtitle: '同室不同床', text: '美国儿科学会建议宝宝出生后至少6个月（最好1年）与父母同房不同床。同房能使SIDS风险降低50%。但不要与宝宝同床——尤其父母饮酒、吸烟、极度疲劳时。' },
    ]
  },
  {
    title: '新生儿常见现象',
    icon: '🔍',
    content: [
      { subtitle: '生理性黄疸', text: '出生2-3天后出现，皮肤眼白发黄，1-2周内自然消退。处理：充分喂养促进排便（大便排胆红素），间接日光浴（隔着玻璃晒）。病理性黄疸症状：24小时内出现、黄疸波及手脚心、持续超过2周、大便发白——需就医。' },
      { subtitle: '吐奶与溢奶', text: '新生儿溢奶（嘴角流出少量奶）是正常现象——贲门括约肌未发育完全。喂奶后竖抱15-20分钟，拍嗝。吐奶呈喷射状或量很大、呈绿色/黄色/带血需就医。如宝宝吐奶但精神好、体重正常增长通常无需过度担心。' },
      { subtitle: '新生儿毒性红斑', text: '出生2-3天后可能出现红色斑点中央有小水疱，像跳蚤咬，可遍布全身。这是正常的新生儿皮疹，不需要治疗，1-2周自行消退。不要挤压或涂抹药膏。' },
      { subtitle: '马牙/上皮珠', text: '牙龈上的白色小点，是角化的上皮细胞。不是真正的牙齿，不需要任何处理，几周内会自行消失。不要用针挑或用力擦。' },
    ]
  },
];

export const babyCareByMonth: Record<string, { title: string; tips: string[] }[]> = {
  '0-1月': [
    { title: '喂养', tips: ['按需喂养，每天8-12次。母乳每次15-20分钟。注意观察饥饿信号。', '如果母乳不足，先亲喂每侧10-15分钟后再补充配方奶。', '妈妈保证足够液体摄入，每天至少2升水。'] },
    { title: '睡眠', tips: ['新生儿每天睡16-18小时，每次持续2-4小时。', '在白噪音或轻柔音乐中入睡。裹襁褓增加安全感（但不要裹太紧影响髋关节发育）。', '昼夜不分是正常的——白天保持明亮，夜间保持昏暗安静。'] },
    { title: '发育', tips: ['俯卧时短暂抬头。注视人脸和对比强烈的图案（黑白卡）。', '每天做婴儿抚触10-15分钟促进感官发育。', '给维生素D 400IU/天（母乳喂养必备）。'] },
  ],
  '1-2月': [
    { title: '喂养', tips: ['继续保持按需喂养。宝宝开始有更规律的进食间隔（约2.5-3小时）。', '注意观察是否出现肠绞痛——每天傍晚/夜间大哭2-3小时，双腿蜷缩。'] },
    { title: '睡眠', tips: ['开始建立简单的睡前程序。观察清醒窗口期（通常45-60分钟）。', '继续裹襁褓但要注意——一旦宝宝能翻身就要停止。'] },
    { title: '发育', tips: ['社交性微笑出现！能与人对视。俯卧时抬头可达45度。', '用摇铃在距离耳朵15-20cm处轻轻摇动训练听觉追踪。'] },
  ],
  '2-3月': [
    { title: '喂养', tips: ['进食间隔约3-4小时。宝宝吃奶效率提高。', '可能出现厌奶期，持续几天是正常的，只要尿量正常、精神好。'] },
    { title: '睡眠', tips: ['开始区分昼夜。建立E.A.S.Y.程序——进食后15-30分钟活动再睡觉。', '使用"昏昏欲睡时放下"的方法——不要等宝宝完全睡着再放。'] },
    { title: '发育', tips: ['俯卧时能抬头90度。双手能握在一起。会笑出声。', '引入不同材质的玩具让宝宝触摸和抓握(布书、硅胶牙胶)。'] },
  ],
};
